...
const tabs = ['All', 'Events', 'Weather', 'Local Insights']

// Icon helper
const getIcon = (type) => {
  if (type === 'google') {
    return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={TEAL} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg>
  }
  if (type === 'weather') {
    return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2563eb" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 16.2A4.5 4.5 0 0 0 17.5 8h-1.8A7 7 0 1 0 4 14.9"/><path d="M8 22v-4"/><path d="M12 22v-4"/><path d="M16 22v-4"/></svg>
  }
  if (type === 'sport') {
    return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ea580c" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M5.4 5.4l13.2 13.2"/><path d="M18.6 5.4L5.4 18.6"/></svg>
  }
  if (type === 'event') {
    return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#9333ea" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
  }
...
// Map the icon to a nice background color
const getIconBg = (type) => {
  if (type === 'weather') return 'rgba(37,99,235,0.08)'
  if (type === 'sport') return 'rgba(234,88,12,0.08)'
  if (type === 'event') return 'rgba(147,51,234,0.08)'
...
export default function CityFeed() {
  ...
  const city = user?.city || user?.restaurant?.city || 'Your City'
  ...
  const fetchTrends = async () => {
    try {
      setLoading(true)
      const res = await trends.getCityTrends(city)
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1000 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <p style={{ fontFamily: 'Space Mono, monospace', fontSize: '0.58rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(12,12,12,0.4)', marginBottom: 4 }}>04 — Engagement Triggers</p>
        <h1 style={{ fontFamily: 'Unbounded, sans-serif', fontWeight: 900, fontSize: '1.6rem', letterSpacing: '-0.03em', color: NAVY, lineHeight: 1 }}>{city} Feed</h1>
      </div>
        ...
        <div style={{ position: 'relative' }}>
          <select
            ...
            style={{
              ...
              appearance: 'none', outline: 'none', opacity: 0.9
            }}
          >
            <option value={city}>{city}</option>
          </select>
          <svg style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', opacity: 0.5 }} width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={NAVY} strokeWidth="2.5" strokeLinecap="round"><polyline points="6 9 12 15 18 9"/></svg>
        </div>
      ...
      {loading ? (
        <div style={{ padding: '40px', textAlign: 'center', fontFamily: 'Inter, sans-serif', color: 'rgba(12,12,12,0.5)' }}>
          Fetching real-time hyper-local data for {city}...
        </div>
      ) : (
        /* Feed cards */
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filtered.length === 0 && (
            <div style={{ padding: '20px', borderRadius: 12, border: '1px solid rgba(12,12,12,0.1)', background: 'rgba(255,255,255,0.7)' }}>
              <p style={{ margin: 0, fontFamily: 'Inter, sans-serif', color: 'rgba(12,12,12,0.6)' }}>
                No active {activeTab.toLowerCase()} data available for {city} right now.
              </p>
              <button onClick={fetchTrends} style={{ marginTop: 12, padding: '8px 12px', borderRadius: 8, border: 'none', background: TEAL, color: 'white', cursor: 'pointer' }}>
                Refresh Feed
              </button>
            </div>
          )}
          {filtered.map(item => {
            ...
            // Prefill prompt based on the trend
